...
class VolumeWeightedMomentumAgent(Agent):
    ...
    async def process(self, message: Message) -> dict | None:
        ...
        if abs(normalized) < 0.5 or vol_ratio < 1.2:
            return None
...
class MultiTimeframeMomentumAgent(Agent):
    ...
    async def process(self, message: Message) -> dict | None:
        ...
        # Strength = geometric mean of absolute momentums
        ...
        await self.emit(
            ...
            "signals",
            {
                ...
                "direction": signal,
                "strength": strength,
                "confidence": self.confidence * 1.1,  # Higher confidence when timeframes align
